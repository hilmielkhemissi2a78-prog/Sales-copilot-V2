...
export default function Resumes() {
  ...
  useEffect(() => {
    fetch('https://sales-copilot-production-0d9c.up.railway.app/resumes')
      .then(res => res.json())
      .then(data => {
        setData(data);
        setLoading(false);
    ...
    <div className="flex h-screen">
      <aside className="w-64 bg-slate-800 text-white p-5 flex flex-col flex-shrink-0">
        ...
        <nav className="flex flex-col gap-2">
          ...
          <Link href="/resumes" className="p-3 rounded bg-blue-600">CV</Link>
          ...
          <div className="grid gap-4">
            {data.map((item: any) => (
              <div key={item.id} className="bg-white p-6 rounded-lg shadow">
                <h2 className="font-bold">{item.name || item.candidateName || 'CV'}</h2>
              </div>
            ))}
          </div>
        )}
